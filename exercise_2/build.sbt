...
testOptions in Test := {
  CrossVersion.partialVersion(scalaVersion.value) match {
    case Some((2, scalaMajor)) if scalaMajor >= 12 => Seq(Tests.Argument("-l","com.example.exercise.tags.UnsupportedInCurrentVersion"))
    case _ => Seq()
  }
}
...
assemblyJarName in assembly := s"exercise_${scalaBinaryVersion.value}.jar"
